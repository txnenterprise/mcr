...
namespace MCR.API.Controllers
{
    ...
    public class ApolicesController : ControllerBase
    {
        ...
        public ApolicesController(IPropostasService propostasService, IPropostasStatusService statusService)
        {
        ...
        {
            request.StatusFilter = new List<string> { "Apólice emitida", "Apólice cancelada" };
        ...
        public async Task<IActionResult> ObterPorId(Guid id)
        {
            ...
            if (proposta == null)
                return NotFound(ApiResponse<object>.Fail("Apólice não encontrada."));
            return Ok(ApiResponse<object>.Ok(proposta));
        }

        [HttpPost("gerar/{cotacaoAgricolaId}")]
        public async Task<IActionResult> GerarApolice(Guid cotacaoAgricolaId)
        {
        ...
        public async Task<IActionResult> EnviarTransmissao(Guid id)
        {
            ...
            if (!result)
                return BadRequest(ApiResponse<object>.Fail("Não foi possível transmitir a apólice."));
            return Ok(ApiResponse<object>.Ok(null, "Apólice transmitida com sucesso."));
        }
