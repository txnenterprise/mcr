...
namespace MCR.API.Controllers
{
    ...
    [Route("api/endossos")]
    [Authorize]
    public class EndossosController : ControllerBase
    {
        private readonly IPropostasService _propostasService;
        private readonly IPropostasStatusService _statusService;

        public EndossosController(IPropostasService propostasService, IPropostasStatusService statusService)
        {
        ...
        public async Task<IActionResult> ObterTodos([FromQuery] ObterPropostasPaginadoRequestDTO request)
        {
            ...
            {
                "Endosso solicitado", "Endosso pendencia", "Endosso transmitido", "Endosso emitido"
            };
        ...
        public async Task<IActionResult> ObterPorId(Guid id)
        {
            ...
            if (proposta == null)
                return NotFound(ApiResponse<object>.Fail("Endosso não encontrado."));
            return Ok(ApiResponse<object>.Ok(proposta));
        }

        [HttpPost("gerar/{cotacaoAgricolaId}")]
        public async Task<IActionResult> GerarEndosso(Guid cotacaoAgricolaId)
        {
        ...
        public async Task<IActionResult> EnviarTransmissao(Guid id)
        {
            ...
            if (!result)
                return BadRequest(ApiResponse<object>.Fail("Não foi possível transmitir o endosso."));
            return Ok(ApiResponse<object>.Ok(null, "Endosso transmitido com sucesso."));
        }
