...
var clienteJS = {
    ...
    initFormValidation: function () {
        ...
        if (form && submitButton) {
            form.addEventListener("submit", function (event) {
                ...
                // Validação de vínculos familiares (comentada no original)
                // const jsonField = document.getElementById("JsonVinculosFamiliar").value;
                // if (!jsonField || jsonField === "[]") {
                //     alert("Adicione pelo menos um vínculo familiar.");
                //     event.preventDefault();
                //     return;
                // }

                // Vínculos de propriedade são opcionais: não exige selectedItems preenchido

                // Desabilita o botão por 3 segundos para evitar envios duplicados
                if (formValido) {
    ...
    initModais: async function () {
        ...
        if (!btnAdicionarPropriedade) {
            console.error("Botão btnAdicionarPropriedade não encontrado!");
            return;
        }
            ...
            .then(data => {
                if (data.success) {
                    notify.success("Talhão Cadastrado", data.message || "Talhão cadastrado com sucesso!", function (d) {
                        ...
                        // Reabrir o modal de propriedade para mostrar o novo talhão
